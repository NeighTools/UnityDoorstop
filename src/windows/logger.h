...
inline void init_logger() {
    ...
    log_handle = CreateFile(buffer, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
}
...
#define ASSERT(test, message)                    \
	if(!(test))                                  \
	{                                            \
		MessageBox(NULL, TEXT(message), TEXT("Doorstop: Fatal"), MB_OK | MB_ICONERROR); \
		ExitProcess(EXIT_FAILURE); \
	}
